...
def get_network(network_config):
    ...
    elif network_config.name == 'oco_net':
        net = OCONet(network_config)
    ...
    if network_config.num_gpus > 1:
        if type(net) is dict:
            ...
            _broadcast = network_config.name != 'oco_net'
            _find_unused = network_config.name == 'oco_net'
            net = torch.nn.parallel.DistributedDataParallel(
                net.cuda(),
                device_ids=[comm.get_local_rank()],
                broadcast_buffers=_broadcast,
