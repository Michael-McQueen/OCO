...
class OODEvaluator(BaseEvaluator):
    ...
    def eval_ood(self,
        ...
        if type(net) is dict:
            ...
            net.eval()
        skip_id_inference = bool(
            getattr(self.config.evaluator, 'skip_id_inference', False))
        if not skip_id_inference:
        ...
        if skip_id_inference:
            print('Skip ID inference (evaluator.skip_id_inference=True). '
                  'Only OOD scores will be generated; OOD metrics are skipped.',
                  flush=True)
            for ood_split in ['nearood', 'farood']:
                ...
                for dataset_name, ood_dl in ood_data_loaders[ood_split].items():
                    ...
                    if self.config.recorder.save_scores:
                        self._save_scores(ood_pred, ood_conf, ood_gt,
                                          dataset_name)
            return
